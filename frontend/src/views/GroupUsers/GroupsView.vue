...
<template>
  <div class="u-p-6">
        ...
        <tr v-for="group in groupsStore.groups" :key="group.id" class="u-hover:bg-gray-50">
          ...
          <td class="u-border u-border-gray-300 u-px-4 u-py-2">
            <div class="u-space-x-2">
              ...
              <button @click="deleteGroup(group.id)" class="u-px-2 u-py-1 u-bg-red-500 u-text-white u-rounded u-text-sm u-hover:bg-red-600">Supprimer</button>
              <button @click="manageFunctions(group)" class="u-px-2 u-py-1 u-bg-green-500 u-text-white u-rounded u-text-sm u-hover:bg-green-600">Gérer Fonctions</button>
            </div>
          </td>
        </tr>
      </tbody>
    ...
    <div v-if="editingGroup" class="u-mt-6 u-p-4 u-border u-border-gray-300 u-rounded">
      ...
      <GroupForm :environmentId="environmentId" :initialData="editingGroup" @success="onFormSuccess" />
    ...
    <div v-if="showFunctionManager" class="u-fixed u-top-0 u-left-0 u-right-0 u-bottom-0 u-bg-black u-bg-opacity-50 u-flex u-justify-center u-items-center">
      <div class="u-bg-white u-p-6 u-rounded-lg u-w-4/5 u-max-w-4xl">
        <h2 class="u-text-xl u-font-bold u-mb-4">Gérer les fonctions du groupe "{{ managingGroup?.name }}"</h2>
        <GroupFunctionsManager :groupId="managingGroup?.id" />
        <button @click="closeFunctionManager" class="u-px-4 u-py-2 u-bg-gray-500 u-text-white u-rounded u-mt-4 u-hover:bg-gray-600">Fermer</button>
      </div>
    </div>
  </div>
</template>
