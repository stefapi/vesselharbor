  ...
  <div class="u-w-full u-max-w-sm u-p-6 u-rounded-xl u-shadow u-bg-white">
    <form @submit.prevent="handlesubmit" class="u-space-y-4">
      ...
      <el-form-item label="Nouveau mot de passe" :error="NewPasswordError">
        <el-input v-model="newPassword" type="password" placeholder="••••••" @blur="v$.newPassword.$touch()" show-password />
      </el-form-item>

      <!-- Confirmation -->
      <el-form-item label="Confirmer le mot de passe" :error="ConfirmPasswordError">
        <el-input v-model="confirmPassword" type="password" placeholder="••••••" @blur="v$.confirmPassword.$touch()" show-password />
      </el-form-item>
...
const validationRules = {
  ...
  confirmPassword: {
    ...
    sameAs: helpers.withMessage('Les mots de passe ne correspondent pas', sameAs(newPassword)),
  },
}
...
const ConfirmPasswordError = computed(() => unref((v$.value.confirmPassword.$error && v$.value.confirmPassword?.$errors[0]?.$message) || ''))
