...
import { login as loginService } from '@/services/userService.ts';
import api from '@/services/api.ts';
import {getUser} from "@/services/userService.ts";

interface UserAssignment {
  group: {
    environment_id: number | null; // Si null, le groupe est global
    ...
    environmentAdminIds: (state): number[] => {
      ...
      const ids = state.user.user_assignments
        .filter(assignment => assignment.group &&
          ...
          assignment.group.environment_id !== null)
  ...
  actions: {
    ...
    async refreshTokenAction() {
      try {
        ...
        const response = await api.post('/refresh-token', { refresh_token: this.refreshToken });
        this.accessToken = response.data.access_token;
        localStorage.setItem('authToken', response.data.access_token);
      } catch (error) {
        this.logout();
